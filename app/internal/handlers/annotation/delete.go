...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	commandTag, err := app.DB.Exec(ctx, fmt.Sprintf("DELETE from %s where id=%s", annotationTableName, id))
	if err != nil {
		log.Warningf("Warning when deleting the element with id %s on table %s: %+v", id, annotationTableName, err)
		helpers.AnswerWith(w, types.ServiceResponse{
	...
	if err := db_ops.DeleteAdditionalPhotos(ctx, id); err != nil {
